...
pub fn batch_add_memories(
    ...
    let target_id = if project_id == "global" {
        ...
        get_or_create_project(None, path, true)?.id
    };
...
pub fn get_memories(
    ...
    let target_id = if project_id == "global" {
        ...
        get_or_create_project(None, path, false)?.id
    };
...
pub fn search_memories(
    ...
    if let Ok(mut stmt) = fts_result {
        let rows = stmt.query_map(params![target_id, fts_query], |row| {
            let tags_str: String = row.get(3)?;
            let meta_str: String = row.get(4)?;
            let perm_int: i32 = row.get(5)?;
    ...
    if result.is_empty() {
        ...
        let rows = stmt.query_map(params![target_id, pattern], |row| {
            let tags_str: String = row.get(3)?;
            let meta_str: String = row.get(4)?;
            let perm_int: i32 = row.get(5)?;
